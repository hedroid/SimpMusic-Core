...
| `/playmode/intelligence/list`（心动模式） | ✅ 可用 | **2026-09-22 复探翻案**（探针 HeartModeProbe 实测 code=200、一次 ~150 首）：weapi 五参数缺一不可——`songId`(种子歌)/`type="fromPlayOne"`/`playlistId`(红心歌单 pid)/`startMusicId`(默认=songId)/**`count`**(如 "1"，字符串数字皆可)；**缺 `count` 即 500 空 message**——首轮探针全灭正是缺 startMusicId/count 所致（当时"三通道×type 全 500"为误判）。响应 `data[]` 每条=`{id,alg,recommended,songInfo{标准歌曲对象}}`，alg 带推荐理由(`sameTag-RH`/`simArtist-RH`)。`/playmode/intelligence/loop` 仍 404。红心电台已切此端点(2026-09-25,旧洗牌方案留 `getHeartRadioContentsLegacyShuffle` 备胎) |
