...
internal class SimpleMediaService :
    ...
    override fun onCreate() {
        ...
        // The old "keep service alive" path posted a SECOND PlayerNotificationManager (id 2026)
        // bound to the same media session and re-asserted startForeground every 30s from a new
        // stacked loop per post. Two MediaStyle notifications fighting over one session made
        // SystemUI drop the real media card and the OEM capsules (Samsung Now Bar) — on devices
        // with that setting on, the provider notification (id 200) vanished entirely. Removed:
        // the DefaultMediaNotificationProvider notification is already ongoing and survives
        // pause, which is all the service needs to stay foreground.
